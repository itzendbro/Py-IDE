/* ============================================================
   ...
   Classic script (works from file://). Waits for CodeMirror
   (js/editor.js) via the "ide-editor-ready" event.
   ============================================================ */
(function () {
  'use strict';

  let editor = null;
  const openTabs = [];   // file node ids
  let activeId = null;
  ...
  function renderTabs() {
    ...
    for (const node of files) {
      ...
      x.textContent = '✕';
      x.title = 'Close';
      x.addEventListener('click', (e) => { e.stopPropagation(); closeTab(node.id); });
      t.appendChild(ic);
      t.appendChild(nm);
      t.appendChild(x);
  ...
  function closeTab(id) {
    ...
    if (activeId === id) activeId = openTabs[Math.min(idx, openTabs.length - 1)] || null;
    renderTabs();
  ...
  function updateStatusLang(name) {
    stLang.textContent = (name.split('.').pop() || '').toUpperCase() || '—';
  }
  ...
  async function runActive() {
    if (!editor) { writeOut('Editor still loading…', 'sys'); return; }
    saveActiveContent();
    window.Files.save();
    const node = getActiveNode();
    if (!node) { writeOut('Open a file to run (create 📄+ a main.py first).', 'sys'); return; }
...
`;
  ...
  window.Files.onDeleted = (id) => { if (openTabs.indexOf(id) >= 0) closeTab(id); };
  window.Files.onRenamed = (node) => {
    if (openTabs.includes(node.id)) { renderTabs(); updateStatusLang(node.name); }
  };
  window.Files.onImported = (count) => writeOut('📥 Imported ' + count + ' file(s).', 'ok');
  ...
  function renderPkg() {
    ...
    for (const p of pkgs) {
      const li = document.createElement('li');
      const nm = document.createElement('span'); nm.className = 'pname'; nm.textContent = p;
      const ok = document.createElement('span'); ok.style.color = 'var(--green)'; ok.textContent = '✓ installed';
      li.appendChild(nm); li.appendChild(ok);
      pkgList.appendChild(li);
    }
  }
  ...
  /* ---------------- wires everything once editor is ready ---------------- */
  function start() {
    editor = new window.IDEEditor(document.getElementById('editor'));
    let saveTimer = null;
    ...
    editor.onCursor = (ln, col) => { stPos.textContent = 'Ln ' + ln + ', Col ' + col; };
    ...
    document.getElementById('fileInput').addEventListener('change', (e) => {
      window.Files.importFiles(e.target.files); e.target.value = '';
    });
    ...
    document.getElementById('exportMenu').addEventListener('click', (e) => {
      ...
      saveActiveContent();
      window.Files.save();
      if (a.dataset.action === 'export-zip') window.Files.exportZip();
    ...
    document.getElementById('clearConsoleBtn').addEventListener('click', () => { consoleEl.innerHTML = ''; });
    ...
    // packages modal
    document.getElementById('pkgBtn').addEventListener('click', () => { renderPkg(); pkgModal.classList.remove('hidden'); });
  ...
  if (window.IDEEditor) start();
  else document.addEventListener('ide-editor-ready', start);
})();
